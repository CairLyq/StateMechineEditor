...
class BehaviorTreeRenderer {
    ...
    renderNode(node) {
        ...
        // 绘制搜索高亮效果
        if (node.isSearchHighlighted) {
            this.renderSearchHighlight(node);
        }
    ...
    renderNodeText(node) {
        ...
        // 文本区域
        const textX = x + width / 2 + 8; // 向右偏移，为图标留出更多空间
        const textY = y + height / 2;
        const maxWidth = width - config.padding * 2 - 32; // 增加图标空间，从24改为32
    ...
    renderSearchHighlight(node) {
        ...
        // 绘制虚线边框
        this.drawRoundedRect(x - 2, y - 2, width + 4, height + 4, 
                           config.borderRadius + 2, null, true);
