...
class BehaviorTreeSimulator {
    ...
    setBlackboardValue(key, value) {
        this.context.blackboard.set(key, value);
        this.logExecution(`设置黑板数据: ${key} = ${JSON.stringify(value)}`, 'debug');
    }
    ...
    clearBlackboard() {
        this.context.blackboard.clear();
        this.logExecution('清空黑板数据', 'debug');
    }

    /**
     * 删除黑板数据
     */
    deleteBlackboardValue(key) {
        const deleted = this.context.blackboard.delete(key);
        if (deleted) {
            this.logExecution(`删除黑板数据: ${key}`, 'debug');
        }
        return deleted;
    }
